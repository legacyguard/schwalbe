...
import { getDocument, updateDocument } from "@/lib/documentsApi";
...
export default function DocumentDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 text-white">
      <div className="mb-4">
        <Link href={`/${locale}/documents`} className="underline text-sky-300">
          ← Back to Documents
        </Link>
      </div>
      <h1 className="text-2xl font-semibold mb-1">{title}</h1>
      <div className="text-slate-300 mb-4">
        {doc.category || "Uncategorized"} • Uploaded {new Date(doc.created_at).toLocaleString()}
      </div>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="border border-slate-700 rounded p-3">
            ...
            <pre className="whitespace-pre-wrap text-sm text-slate-200">
              {doc.ocr_text || doc.ai_extracted_text || "No text extracted."}
            </pre>
          </div>
          <div className="border border-slate-700 rounded p-3">
          ...
          <div className="border border-slate-700 rounded p-3">
            ...
            <div className="space-y-2">
              ...
              <input
                className="w-full rounded bg-slate-800 border border-slate-700 p-2"
                defaultValue={exp || ""}
                onChange={(e) => setEdit((s) => ({ ...s, expiration_date: e.target.value }))}
              />
              <button
                disabled={saving}
                className="mt-2 px-3 py-2 rounded bg-slate-700 hover:bg-slate-600"
                onClick={onAccept}
              >
                {saving ? "Saving…" : "Save"}
              </button>
            ...
            <div className="text-sm text-slate-300">
              Category: {doc.category || "—"} {doc.classification_confidence != null ? `(${Math.round(doc.classification_confidence * 100)}%)` : ""}
            </div>
            <div className="text-sm text-slate-300">
              Title: {doc.title || "—"} {doc.ai_confidence != null ? `(${Math.round(doc.ai_confidence * 100)}%)` : ""}
            </div>
            {doc.ai_reasoning ? (
