...
import { DOMAIN_LANGUAGES } from './config/domains'
...
function getAllowedLanguagesForHost(hostname: string): string[] {
  ...
  return (
    EXTENDED_DOMAIN_LANGUAGES[hostname] ||
...
export default function middleware(request: NextRequest) {
  ...
  // Test-only E2E cookie (enable deterministic drivers)
  try {
    const enableE2E = process.env.NEXT_PUBLIC_E2E === '1' || request.nextUrl.searchParams.get('e2e') === '1'
    if (enableE2E && (response as any)?.cookies) {
      ;(response as any).cookies.set('e2e', '1')
    }
  } catch {}
