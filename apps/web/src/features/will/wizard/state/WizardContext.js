...
export function WizardProvider({ children }) {
    ...
    const [state, setState] = useState(() => {
        ...
        return local ? JSON.parse(local) : InitialWizardState;
    });
    ...
    const saveDraft = useCallback(async (opts) => {
        ...
        if (!userId) {
            ...
            localStorage.setItem('will_wizard_state', JSON.stringify(state));
            if (opts?.toast)
                opts.toast('Draft saved locally');
            return;
        }
        ...
        if (error) {
            ...
            localStorage.setItem('will_wizard_state', JSON.stringify(state));
            if (opts?.toast)
                opts.toast('Draft saved locally (offline mode)');
            return;
        }
        if (opts?.toast)
            opts.toast('Draft saved');
    }, [currentStep, sessionId, state]);
    const loadDraft = useCallback(async (overrideSessionId) => {
        const { data: auth } = await supabase.auth.getUser();
        const userId = auth.user?.id;
