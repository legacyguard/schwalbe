...
- apps/
  - web (Next.js App Router, SSR/RSC, Edge runtime for server routes)
...
- Create Next.js app in apps/web (keep Vite app temporarily if needed, but do not extend it)
...
- Adopt single i18n layer for Next (e.g., next-intl or next-i18next)
...
- Sofia presence (firefly motion) and a guided dialog surface (non-LLM stub OK initially)
- 3‑act onboarding (reduce anxiety; celebrate clarity and action)
...
- Vite web app (apps/web) — keep only while bootstrapping Next.js; do not extend it
