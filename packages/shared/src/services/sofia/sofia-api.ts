...
class SofiaAPI {
  ...
  constructor() {
    this.supabaseUrl = typeof window !== 'undefined' ? (import.meta as any).env?.VITE_SUPABASE_URL || '' : process.env.VITE_SUPABASE_URL || '';
    this.supabaseKey = typeof window !== 'undefined' ? (import.meta as any).env?.VITE_SUPABASE_ANON_KEY || '' : process.env.VITE_SUPABASE_ANON_KEY || '';
