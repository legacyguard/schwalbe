...
export const SofiaFirefly: React.FC<SofiaFireflyProps> = ({
  ...
  // For now, we'll use default values since we don't have the full context system yet
  const effectiveIsVisible = isVisible;
  const effectiveTargetElement = targetElement;
  const effectiveCelebrateEvent = celebrateEvent;

  // Default user preferences - will be replaced with actual user preferences later
  const [userPreferences] = useState({
    communication: {
      style: 'empathetic' as CommunicationStyle,
    },
  });
  ...
  }, [activeMode, controls, effectiveIsVisible, updateTrail]);
