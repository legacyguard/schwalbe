...
interface MilestoneAchievementBadgesProps {
  achievedMilestones: EnhancedMilestone[];
  className?: string;
  onBadgeClick?: (milestone: EnhancedMilestone) => void;
  showUpcoming?: boolean;
  upcomingMilestones?: EnhancedMilestone[];
...
export function MilestoneAchievementBadges({
  ...
  const getMilestoneIcon = (type: EnhancedMilestone['type']) => {
    switch (type) {
      case 'first_document':
        return FileCheck;
  ...
  const getBadgeGradient = (
    color: EnhancedMilestone['badge_data']['color'],
    achieved: boolean
  ) => {
  ...
  const getRarityEffects = (
    rarity: EnhancedMilestone['badge_data']['rarity']
  ) => {
    switch (rarity) {
      case 'legendary':
        return {
  ...
  const renderBadge = (data: BadgeDisplayData, index: number) => {
    ...
    return (
      <div key={data.milestone.id} className="relative group">
        <motion.div
        ...
        <div className='absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50'>
          <div className='bg-gray-900 text-white text-sm rounded-lg p-3 shadow-lg max-w-64'>
            <div className='flex items-center gap-2 mb-1'>
              <h4 className='font-semibold text-sm'>
                {data.milestone.title}
              </h4>
              {!data.achieved && <Lock className='h-3 w-3 text-gray-400' />}
            </div>
