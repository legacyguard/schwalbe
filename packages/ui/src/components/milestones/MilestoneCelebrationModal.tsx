...
interface MilestoneCelebrationModalProps {
  ...
  onClose: () => void;
  onShareMilestone?: (milestone: EnhancedMilestone) => void;
}
...
export function MilestoneCelebrationModal({
  ...
  const getBadgeColor = (color: EnhancedMilestone['badge_data']['color']) => {
    switch (color) {
      case 'gold':
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${className}`}>
      ...
      {/* Modal Content */}
      <motion.div
        className='relative bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl shadow-2xl p-0 overflow-hidden border-0 max-w-md w-full mx-4'
      ...
      >
        <div className='relative'>
          ...
          >
            ...
            >
              <div
                className={`absolute inset-0 rounded-full ${getBadgeColor(milestone.badge_data.color)}`}
              />
