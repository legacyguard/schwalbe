...
Note: RLS policies use `auth.uid()` as the identity source (Supabase Auth user ID). Where user_id is present, ensure it references `auth.users(id)` in relational schemas.
